...
function FeaturedProjectCarousel({ projects }: { projects: any[] }) {
  ...
  return (
    <div className="mb-7 relative group rounded-2xl overflow-hidden shadow-xl border-2 border-violet-700">
      {current.video
        ? <video autoPlay className="w-full h-60 object-cover" loop muted playsInline src={current.video}></video>
      ...
      <div className="absolute right-9 bottom-3 flex gap-1">
        {featured.map((_,i) => <span className={`inline-block w-2 h-2 rounded-full ${i===idx? 'bg-violet-400':'bg-zinc-600'}`} key={i}></span>)}
      </div>
    </div>
  );
}
  ...
  return (
    <AnimatePresence>
      <motion.div className="fixed inset-0 bg-black/70 flex z-50 items-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        <motion.div className="bg-zinc-950 border-2 border-violet-700 max-w-lg w-[97vw] rounded-2xl p-8 shadow-2xl relative text-zinc-100" initial={{ scale: .9, y: -60, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: .95, y: 60, opacity: 0 }}>
          ...
          {project.video ? <video className="w-full h-48 rounded-xl mb-3 object-cover" controls src={project.video}></video> : <img alt="" className="w-full rounded-xl mb-3 object-cover" src={project.image}/>}
...
function SVGBackground() {
  return (
    <svg className="fixed inset-0 z-0 w-full h-full pointer-events-none" viewBox="0 0 1440 850" fill="none" style={{ opacity: .41 }}>
      <path d="M0,720 Q360,650 720,900 Q1080,1150 1440,800 V0 H0 Z" fill="url(#a)" fillOpacity={.8} />
      <defs>
    ...
    <div className="min-h-screen flex flex-row bg-zinc-950 dark:bg-zinc-950 text-zinc-100 overflow-x-hidden font-mono">
      ...
      <aside className="hidden md:flex w-[265px] sticky top-0 min-h-screen flex-col z-40 border-r bg-zinc-900/90 border-zinc-800 py-8 gap-8">
        ...
        <div className="flex gap-4 px-6 mt-auto items-center">
          {socialLinks.map(s =>
            <a href={s.url} key={s.name} rel="noopener" target="_blank">
      ...
      <main className="flex-1 flex flex-col min-h-screen relative z-10">
        {/* HERO */}
        <motion.section id="hero" className="pt-11 pb-16 px-5 md:px-20 min-h-[330px] md:min-h-[380px]" initial={{ opacity: 0, y: 20 }} animate={{ opacity: .8, y: 0 }} transition={{ type: 'spring', duration: .8 }}>
          <div className="max-w-2xl">
            ...
            <motion.p className="mt-1 text-base text-zinc-200 max-w-lg" initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{delay:.45}}>Young Cloud/Security/DevOps architect with a passion for automation, SIEM, and modern fullstack projects. <span className="hidden md:inline">Looking for my first IT job—let’s connect!</span></motion.p>
            <div className="flex gap-5 mt-7">
              ...
              <a className="rounded-xl px-7 py-3 text-indigo-900 bg-white/90 font-bold border border-indigo-200 hover:bg-violet-100 shadow hover:scale-105 transition" download href="/resume.pdf">Download CV</a>
            </div>
          </div>
        </motion.section>
        {/* --- FEATURED CAROUSEL + GRID --- */}
        <motion.section id="work" className="w-full mb-8 px-5 md:px-20" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: .9, y: 0 }} transition={{ type: 'spring', duration: .9 }}>
